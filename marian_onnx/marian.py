...
from transformers import MarianConfig, GenerationMixin, LogitsProcessor

from marian_onnx.utils import create_model_for_provider
...
class CustomLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, min_length: int, eos_token_id: int, pad_token_id: int):
        ...
        if not isinstance(pad_token_id, int) or pad_token_id < 0:
            raise ValueError(
                f"`pad_token_id` has to be a positive integer, but is {pad_token_id}"
            )
...
class MarianOnnx(GenerationMixin):
    def __init__(self, path: str, device: str = "cpu"):
        ...
        self.final_logits_weight = torch.load(os.path.join(path, "lm_weight.bin")).to(
            self.device
        )
        self.final_logits_bias = torch.load(os.path.join(path, "lm_bias.bin")).to(
            self.device
        )
    ...
    def _prepare_decoder_input_ids_for_generation(
        self, input_ids, decoder_start_token_id: int
    ):
        decoder_input_ids = (
            torch.ones(
